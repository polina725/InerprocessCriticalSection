...
LPCTSTR initSomeMapping(HANDLE* mapping, size_t size, LPCWSTR name);
...
int main()
{
    ...
    std::wstring CommandLineFirst(L"FirstProc.exe");
    std::wstring CommandLineSecond(L"SecondProc.exe");
    std::wstring CommandLineThird(L"ThirdProc.exe");
    ...
    bufForCrSection = initSomeMapping(&mappingForCrSection, sizeOfSection, CRITICAL_SECTION_NAME);
    if (bufForCrSection == NULL) {
        return -1;
    }

    bufForSharedMemory = initSomeMapping(&mappingForMemory, sizeOfSharedMemory, SHARED_MEMORY_NAME);
